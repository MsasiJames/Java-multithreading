...
public class MainSimulation {
    ...
    public static boolean machineBroken = false;
    ...
    public static class Customer extends Thread {
        ...
        public void run() {
            try {
                ...
                try{Thread.sleep(ThreadLocalRandom.current().nextInt(1000,3000));} catch (InterruptedException e) {} // simulate entering gate
                getInTerminal();
        ...
        private synchronized void getInTerminal() throws InterruptedException{
            ...
            }else if((CustomerId >= 40 && CustomerId < 80) && !(machineBroken)){
                if(boothTwo.size() == 20){
                    boothTwo.remove();
                }
        ...
        private synchronized void BoardingBus() throws InterruptedException{     // assumption is that all the buses leave together
    ...
    public static void main(String[] args) throws InterruptedException {
        Schedulers scheduleThreads = new Schedulers();  // this has to be in this position.
        scheduleThreads.scheduleTask(new Runnable() {
            @Override
            public void run() {
                scheduleThreads.bathRoomAction();   // scheduled thread for bathroom break
            }
        }, 7, TimeUnit.SECONDS);
        ...
        scheduleThreads.scheduleTask(new Runnable() {
            @Override
            public void run() {
                Schedulers.executeMachineBreak();   // scheduled thread for breaking machine
            }
        }, 40, TimeUnit.SECONDS);
        ...
        scheduleThreads.scheduleTask(new Runnable() {
            @Override
            public void run() {
                Schedulers.executeMachineFix();     // scheduled thread for fixing machine
            }
        }, 45, TimeUnit.SECONDS);
        ...
        for (int i = CUSTOMER_MAX / 2; i < CUSTOMER_MAX; i++) {     // this was done so as no two customers would have the same Id from east and west
            ...
            custEast.start();
            custEast.join();    // these threads have to complete first before the thread in line 288
        }

        try {scheduleThreads.shutdown();} catch (Exception e) {}
